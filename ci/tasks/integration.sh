...
scp -i ssh.key -o "StrictHostKeyChecking no" -r voyager-houston  ${INTEGRATION_VM_USER}@${INTEGRATION_VM_IP}:$project_path

integration_file=$PWD/voyager-houston/ci/tasks/run-integration.sh
